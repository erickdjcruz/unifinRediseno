({
	...
	events: {
		...
		'click #validar_CIEC': 'validarServicioCIEC',
	...
	formatearFecha: function (fechaISO) {
		...
		// Redondeo de minutos a 00 o 30
		minutos = minutos < 30 ? 0 : 30;
	...
	validarServicioCIEC: function () {
		...
		if (contexto_cuenta.model.get('rfc_c') != '' && contexto_cuenta.model.get('rfc_c') != undefined) {
			...
			app.api.call('create', app.api.buildURL("GetInfoRFCbyCIEC"), body, {
				success: _.bind(function (data) {
					...
					if (data.success) {
						...
						/********* campos buro *****/								
						var RichedCity = data["address"]["riched_city"].toUpperCase();
						var RichedColonia = data["address"]["riched_neighborhood"].toUpperCase();
						var Localidad = data["address"]["locality"].toUpperCase();
											...
											onConfirm: function () {
												...
												app.api.call('GET', app.api.buildURL(strUrl), null, {
													success: _.bind(function (data) {
														...
														var direccionBuro = cont_dir.oDirecciones.direccion;																
														...
														var auxEstadoFiscal = '';
														var auxEstadoFLabel = '';
														var listEstadoFiscal = {};
														//Municipio
														var auxMunicipioFiscal = '';
														var auxMunicipioFLabel = '';
														var listMunicipioFiscal = {};
														//Colonia
														var auxColLocalId = '';
														var auxColLocalFiscal = '';
														var listColLocalFiscal = {};
														//Ciudad
														var auxCiudadMuniId = '';
														var auxCiudadMunFiscal = '';
														var listCiudadMunFiscal = {};
														/***********************/
														var newaddBuro = 0;
														...
														if ( contexto_dire_buro.direccionBuro.length == 0){
															...
															/***********************/
															//Itera para validar diferencia en dirección fiscal
															Object.keys(direccionBuro).forEach(key => {
																//Valida dirección Buro de credito
																if (direccionBuro[key].indicadorSeleccionados.includes('64') && direccionBuro[key].inactivo == 0) {
																	tieneBuro = true;
																}
																//Valida dirección fiscal
																if (direccionBuro[key].indicadorSeleccionados.includes('^2^') && direccionBuro[key].inactivo == 0) {
																	keyFiscal = key;

																	CPFiscal = direccionBuro[key].valCodigoPostal;
																	idCPFiscal = direccionBuro[key].postal;
																	CalleFiscal = direccionBuro[key].calle;
																	ExteriorFiscal = direccionBuro[key].numext;
																	InteriorFiscal = direccionBuro[key].numint;
																	//Pais
																	auxPaisFiscal = direccionBuro[key].pais;
																	auxPaisFLabel = listPais[direccionBuro[key].pais];
																	listPaisFiscal = listPais;
																	//Estado
																	auxEstadoFiscal = direccionBuro[key].estado;
																	auxEstadoFLabel = listEstado[direccionBuro[key].estado];
																	listEstadoFiscal = listEstado;
																	//Municipio
																	auxMunicipioFiscal = direccionBuro[key].municipio;
																	auxMunicipioFLabel = listMunicipio[direccionBuro[key].municipio];
																	listMunicipioFiscal = listMunicipio;

																	//Colonia
																	if( sincolonia.includes(RichedColonia)){
																		newaddBuro = 1;	
																		RichedColonia = Localidad;
																	}else{
																		auxColLocalId = direccionBuro[key].colonia; 
																		auxColLocalFiscal = Object.values(listColonia).find(c => c.idColonia === auxColLocalId)?.nameColonia;
																		listColLocalFiscal = listColonia;
																		if(contextol.normalizarTexto(RichedColonia) != contextol.normalizarTexto(auxColLocalFiscal)){
																			newaddBuro = 1;
																		}	
																	}
																																				
																	//Ciudad
																	if( sinciudad.includes(RichedCity)){
																		newaddBuro = 1;	
																		RichedCity = listMunicipio[direccionBuro[key].municipio];																																	
																	}else{
																		auxCiudadMuniId = direccionBuro[key].ciudad;
																		auxCiudadMunFiscal =  listCiudad[auxCiudadMuniId];
																		listCiudadMunFiscal = listCiudad;
																		if(contextol.normalizarTexto(RichedCity) != contextol.normalizarTexto(auxCiudadMunFiscal)){																			
																			newaddBuro = 1;
																		}	
																	}																																				
																}
															...
															if(!tieneBuro){
																...
																// Consulta direccion buro
																var strUrl = 'DireccionesQR/' + CPFiscal + '/0/' + RichedColonia + '/' + auxMunicipioFLabel + '/' + auxEstadoFLabel + '/' + RichedCity;
																strUrl = strUrl.replaceAll(' ', '+');
																	...
																	success: function (data) {
																		...
																		if (data.idCP) {
																			...
																			for (var i = 0; i < data.paises.length; i++) {
																				listPais[data.paises[i].idPais] = data.paises[i].namePais;
																				auxPais = data.paises[i].idPais;
																			}
																			...
																			for (var i = 0; i < data.estados.length; i++) {
																				listEstado[data.estados[i].idEstado] = data.estados[i].nameEstado;
																				auxEstado = data.estados[i].idEstado;
																			}
																			...
																			resultadodireBuro = {
																				auxPais: auxPais,
																				auxEstado: auxEstado,
																				auxMunicipio: auxMunicipio,
																				auxColonia: auxColonia,
																				auxCiudad: auxCiudad,
																				idSinCiudad: idSinCiudad,
																				listPais: listPais, 
																			...
																			/************************************ */
																			/******** Ajuste Dirección buro CP ************/																				
																			nuevoBuro.listTipo = App.lang.getAppListStrings('dir_tipo_unique_list');
																			nuevoBuro.secuencia = secuenciaburo;
																			nuevoBuro.tipodedireccion = "1";
																			nuevoBuro.tipoSeleccionados = '^1^';
																			nuevoBuro.indicador = "64";
																			nuevoBuro.indicadorSeleccionados = '^64^';
																			nuevoBuro.bloqueado = 0;
																			nuevoBuro.valCodigoPostal = CPFiscal ;
																			nuevoBuro.postal = data.idCP;
																			nuevoBuro.calle = CalleFiscal ;
																			nuevoBuro.numext = ExteriorFiscal ;
																			nuevoBuro.numint = InteriorFiscal ;

																			//Pais
																			nuevoBuro.pais = resultadodireBuro.auxPais;
																			nuevoBuro.listPais = resultadodireBuro.listPais;
																			nuevoBuro.listPaisFull = resultadodireBuro.listPais;
																			//Estado
																			nuevoBuro.estado = resultadodireBuro.auxEstado;
																			nuevoBuro.listEstado = resultadodireBuro.listEstado;
																			nuevoBuro.listEstadoFull = resultadodireBuro.listEstado;
																			//Municipio
																			nuevoBuro.municipio = resultadodireBuro.auxMunicipio;
																			nuevoBuro.listMunicipio = resultadodireBuro.listMunicipio;
																			nuevoBuro.listMunicipioFull = resultadodireBuro.listMunicipio;
																			//Colonia
																			nuevoBuro.colonia = resultadodireBuro.auxColonia;
																			nuevoBuro.listColonia = resultadodireBuro.listColonia;
																			nuevoBuro.listColoniaFull = resultadodireBuro.listColonia;
																			//Ciudad
																			nuevoBuro.ciudad = resultadodireBuro.auxCiudad;
																			nuevoBuro.listCiudad = resultadodireBuro.listCiudad;
																			nuevoBuro.listCiudadFull = resultadodireBuro.listCiudad;
																			nuevoBuro.id_sepomex = data.idCP;
																			/*
																			if(newaddBuro){
																				//Pais
																				nuevoBuro.pais = resultadodireBuro.auxPais;
																				...
																				//Pais
																				nuevoBuro.pais = auxPaisFiscal ;
																				nuevoBuro.listPais = listPaisFiscal ;
																				nuevoBuro.listPaisFull = listPaisFiscal ;
																				//Estado
																				nuevoBuro.estado = auxEstadoFiscal ;
																				nuevoBuro.listEstado = listEstadoFiscal ;
																				nuevoBuro.listEstadoFull = listEstadoFiscal;
																				//Municipio
																				nuevoBuro.municipio = auxMunicipioFiscal ;
																				nuevoBuro.listMunicipio = listMunicipioFiscal  ;
																				nuevoBuro.listMunicipioFull = listMunicipioFiscal   ;
																				//Colonia
																				nuevoBuro.colonia = auxColLocalFiscal ;
																				nuevoBuro.listColonia = listColLocalFiscal ;
																				nuevoBuro.listColoniaFull = listColLocalFiscal ;
																				//Ciudad
																				nuevoBuro.ciudad = auxCiudadMunFiscal ;
																				nuevoBuro.listCiudad = listCiudadMunFiscal ;
																				nuevoBuro.listCiudadFull = listCiudadMunFiscal ;																					
																			}
																			*/
																			/*******************************************/																				
																			contexto_dire_buro.direccionBuro.push(nuevoBuro);
																			contexto_dire_buro.prev_direccionBuro = app.utils.deepCopy(contexto_dire_buro.direccionBuro);
																			//contexto_dire_buro.prev_direccionBuro = contexto_dire_buro.direccionBuro;
																			contexto_dire_buro.render();			
																		}
																	},
																	error: function (err) {
																		...
																		app.alert.show('errorCSF', {
																			level: 'error',
																			messages: 'Error en dirección de buro',
																			autoClose: true
																		});
																	}
																});																																		
